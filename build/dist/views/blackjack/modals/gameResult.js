...
export class GameResultModal {
  static createGameResultModal(resultLog, chips) {
    const numOfRounds = resultLog.length;
    let chipsDelta = chips - INITIAL_CHIPS;
    let plusMinusSymbol = chipsDelta >= 0 ? "+" : "-";
  ...
  static createResultLogTableRow(resultLog) {
  ...
  static setHomeButtonEvent() {
    const homeButton = document.querySelector("#homeButton");
    homeButton.addEventListener("click", () => {
      Controller.goHome();
    });
  }
  static setNewGameButtonEvent(table) {
    const homeButton = document.querySelector("#newGameButton");
    homeButton.addEventListener("click", () => {
